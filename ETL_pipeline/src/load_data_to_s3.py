...
def df_to_s3(df, key, s3_bucket, aws_access_key_id, aws_secret_access_key):
    ...
    if file_type == ".csv":
        buffer = StringIO()  # create buffer to temporarily
        # store the Data Frame
        df.to_csv(buffer, index=False)  # code to write the data
        # frame as .csv file
    ...
    print(
        f"The transformed data is saved as {file_type} "
        f"in the following location s3://{s3_bucket}/{key}"
    )
